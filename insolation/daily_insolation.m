function [Fsw,ecc,obliquity,long_perh] = daily_insolation(kyear,lat,day,day_type,So)
...
if day_type>=0
    ...
    if length(kyear)~=3
        disp('Error: expect 3-element kyear argument for day_type<0'), Fsw=nan; daily_insolation; return
    end
    ecc=kyear(1);
...
elseif abs(day_type)==2 %solar longitude (1-360)
    ...
    disp('Error: invalid day_type'); Fsw=nan; daily_insolation; return
end
...
% Insolation: Bischoff et al. (2016)
%Fsw=So/pi.*(1+2*ecc.*cos(lambda-omega)) .* ...
%    ( pi/2.*sin(lat).*epsilon.*sin(lambda) + cos(lat) );